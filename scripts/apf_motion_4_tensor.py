...
class PlannerRT(object):
    ...
    def __init__(self, model, robot, init_params):
        ...
        # listener
        self.check_topic()
        rospy.Subscriber(self.p.topic, self.p.topic_type, self.get_odom)
    ...
    def init(self, model, robot, init_params):
        ...
        self.v = 0
        self.w = 0
        self.p.v_max = 0.2        # init_params.v_max
        self.p.v_min = 0.0        # init_params.v_min
        self.p.w_min = 0.0        # init_params.w_min
        self.p.w_max = 1.0        # init_params.w_max
        self.p.v_min_2 = 0.05     # init_params.v_min_2
    ...
    def cal_vel(self, f_r, f_theta, theta):
        ...
        if f_r < 0:
            ...
            v = 1 * self.p.v_max * \
                ((f_r / self.p.fix_f)**2)  # + self.p.v_min_2
            # v = 1 * self.p.v_max * ((f_r / self.p.fix_f)**2 + (f_r / self.p.fix_f) / 4) # + self.p.v_min_2
    ...
    def detect_group(self):
        ...
        if len(robots_inds) == 0:
            if len(multi_robots_vis) > 0:
                ...
                self.vs.draw_robot_circles(multi_robots_vis, self.p.ns)
            return
    ...
    def check_topic(self):
        self.p.topic_msg = None
        rospy.loginfo(self.p.ns + " apf_motion, checking topic ...")
        while self.p.topic_msg is None:
            try:
                self.p.topic_msg = rospy.wait_for_message(
                    self.p.topic, self.p.topic_type, timeout=3.0)
        ...
        position = self.p.topic_msg.pose.pose.position
        quaternion = self.p.topic_msg.pose.pose.orientation
        orientation = euler_from_quaternion(
            [quaternion.x, quaternion.y, quaternion.z, quaternion.w])
        self.r_x = position.x
        self.r_y = position.y
        self.r_h = orientation[2]
        return self.p.topic_msg
    ...
    def tensor_force(self):
        ...
        if flag_xy:
            ...
            for i in range(X_outside.shape[0]):
                for j in range(X_outside.shape[1]):
                    if X_outside[i, j] == np.nan or Y_outside[i, j] == np.nan:
                        ...
                        r_h = self.cal_theta(
                            X_outside[i, j], Y_outside[i, j], N)
                        Fx[i, j], Fy[i, j] = self.f_obstacle_tensor(
                            X_outside[i, j], Y_outside[i, j], N, r_h)
                        if flag_robot:
                            fr_x, fr_y = self.robot_force_tensor(
                                X_outside[i, j], Y_outside[i, j], r_h)
                        ft_x, ft_y = self.tensor_target(
                            X_outside[i, j], Y_outside[i, j], r_h)
                        if Fx[i, j] > 50 or Fy[i, j] > 50:
                            Fx[i, j], Fy[i, j], thetaF[i,
            ...
            normalized_force_tensor = force_tensor / \
                force_magnitude[..., None] * 1.0
            ...
            for rr in r:
                ...
                for tt in t:
                    ...
                    if abs(fx + fr_x + ft_x) > 50 or abs(fy + fr_y + ft_y) > 50:
                        Fx[i, j], Fy[i, j], thetaF[i, j] = np.nan, np.nan, np.nan
                    else:
                        Fx[i, j] = fx + fr_x + ft_x
                        Fy[i, j] = fy + fr_y + ft_y
    ...
    def robot_force_tensor(self, x, y, h):
        ...
        if len(self.new_robots) > 0:
            ...
            if (nr.d < nr.r_start):
                ...
                # forces
                f = ((nr.z * 1) * ((1 / nr.d) - (1 / nr.r_start))**2) * \
                    (1 / nr.d)**2
                fl = f + 0.5
                nr_force = [fl * -np.cos(ad_h_rR), fl * np.sin(ad_h_rR)]
