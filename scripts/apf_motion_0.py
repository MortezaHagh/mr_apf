...
class ApfMotion(object):
    def __init__(self, model, robot, init_params):
        ...
        self.theta_thresh = 30*np.pi/180 #init_params.theta_thresh
    ...
    def cal_vel(self, f_r, f_theta, theta):
        ...
        # else:
        theta_thresh = 30*np.pi/180
        v = self.v_max * max(0, (1- (abs(theta)/theta_thresh)))**1 #+ self.v_min/2
        ...
        for i in range(resp.count):
            ...
            if d_ro >= 2*robot_r:
                ...
                if d_ro > robot_r:
                    ...
                    f = 4 + ((self.zeta*1)*((1/d_ro)-(1/robot_r))**2)*(1/d_ro)**2 
                    templ = [f*np.cos(angle_diff), f*np.sin(angle_diff)]
        ...
        for i in range(self.obs_count):
            ...
            if d_ro >= 2*obs_effect_r:
                ...
                if d_ro > obs_effect_r:
                    f = 4 * (2*obs_effect_r-d_ro)/obs_effect_r
                    templ = [f*np.cos(angle_diff), f*np.sin(angle_diff)]
                    if angle_diff>0:
                        phi = theta-np.pi/2
                    else:
                    ...
                    f = 4 + ((self.zeta*1)*((1/d_ro)-(1/obs_effect_r))**2)*(1/d_ro)**2 
                    templ = [f*np.cos(angle_diff), f*np.sin(angle_diff)]
