...
class RobotPlanner(object):
    ...
    def exec_cb(self, goal):
        ...
        self.res.x = self.pd.x
        self.res.y = self.pd.y
    ...
    def go_to_goal(self):
        ...
        while self.goal_distance > self.dis_tresh and not rospy.is_shutdown():
            ...
            # result
            self.pd.x.append(self.r_x)
            self.pd.y.append(self.r_y)
    ...
    def get_robot(self):
        self.r_x = self.model.robots[self.rid].xs
        self.r_y = self.model.robots[self.rid].ys
        self.r_theta = self.model.robots[self.rid].heading
    ...
    def map_data(self):
        # robot target
        self.goal_x = self.model.robots[self.rid].xt
        self.goal_y = self.model.robots[self.rid].yt
